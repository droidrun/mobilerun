...
llm = load_llm("GoogleGenAI", model="gemini-3.8-flash")
...
agent = MobileAgent(
    goal="...",
    llms={
        "manager": load_llm("OpenAIResponses", model="gpt-6-astra"),  # Planning
        "executor": load_llm("GoogleGenAI", model="gemini-3.8-flash"),  # Action selection
        "fast_agent": load_llm("GoogleGenAI", model="gemini-3.8-flash"),   # Fast Agent: Direct execution (XML tool-calling)
        "app_opener": load_llm("OpenAIResponses", model="gpt-6-sol"),  # App launching
        "structured_output": load_llm("GoogleGenAI", model="gemini-3.8-flash"),  # Output extraction
    }
)
```
...
mobilerun run "Task" --provider GoogleGenAI --model gemini-3.8-flash
...
`MiniMax-M2.7` model with `https://api.minimaxi.com/v1`. `MiniMax-M3` is the only
MiniMax model that reads screenshots; the M2.x models ignore image input.
