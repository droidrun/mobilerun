...
| OrcaRouter | Included by default | `ORCAROUTER_API_KEY` |
...
reaches models from several vendors through one key, using `vendor/model` ids.
It is a first-class provider family, so `mobilerun configure` prefills
`https://api.orcarouter.ai/v1` and offers a starting model list; any model id the
account can reach is accepted.

```bash
export ORCAROUTER_API_KEY=your-key
mobilerun configure --provider orcarouter --model anthropic/claude-sonnet-5
```

It can also be selected per run without the wizard:

```bash
mobilerun run "Open settings and turn on airplane mode"   --provider OrcaRouter   --model openai/gpt-5.6-sol   --base_url https://api.orcarouter.ai/v1
```
...
| `ORCAROUTER_API_KEY` | OrcaRouter API key | None |
