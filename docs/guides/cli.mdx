...
mobilerun configure --provider XAI --auth-mode oauth --model grok-4.7
...
With an API key, Grok 4.6 is the model. With OAuth, Grok 4.7 is the default and
Grok 4.6 remains available by selecting `--model grok-4.6` explicitly.
...
still-supported `MiniMax-M2.7` model. M2.x models ignore screenshots, so use
them without vision mode:
